...
Projekt je záměrně postaven jako analytické řešení využívající především:
...
Architektura odpovídá rozsahu projektu, objemu přibližně 250 tisíc zásilek a cíli procvičit pokročilejší analytickou práci v Excelu a Power Query bez zavádění další zbytečné infrastruktury.

## Role Power Query

Power Query zajišťuje:
- načtení 24 měsíčních CSV souborů;
- kombinaci jednotlivých měsíčních exportů;
- načtení dimenzionálních/referenčních tabulek;
- sjednocení datových typů;
- standardizaci textových hodnot;
- validaci referenčních klíčů;
- odstranění duplicit;
- validaci business pravidel;
- propojení referenčních dat;
- agregaci stížností oproti zásilce;
- tvorbu odvozených analytických atributů;
- přípravu finální analytické tabulky.

## Forma úložiště

Hlavním úložištěm jsou soubory. Raw vrstva je uložena v CSV souborech v projektové struktuře. Pro tento objem dat a měsíční reporting není zavedení SQL databáze, Data Warehouse ani Data Lake nutné.
...
Projekt nepoužívá scheduler ani externí pipeline. Samotné obnovení je v rámci portfolio projektu spouštěno ručně v Excelu.

Část je automatizována prostřednictvím Power Query:
- zásilkové soubory se načítají ze složky;
- nový měsíční CSV soubor může být zahrnut při obnovení dat bez ručního přidání;
- transformační kroky se při aktualizaci opakují automaticky.

## Zamítnuté alternativy

### SQL databáze

Pro tento projekt nebyla zvolena, protože by nepřinesla odpovídající přidanou hodnotu a zbytečně by zvýšila technickou složitost.

### Python pro cleaning a transformace

Python by zvládl stejný typ transformací, ale cílem tohoto projektu je prokázat schopnost práce v Excelu a Power Query.

### Power BI

Power BI nebylo zvoleno, protože cílem prvního projektu je management reporting v Excelu.
...
Chybějící hodnoty nebyly automaticky nahrazovány umělými hodnotami a byly rozlišeny podle jejich business významu.
...
Tyto hodnoty nebyly doplňovány např. mediánem. Řádky zůstávají v datasetu, ale chybějící údaj se nepoužije při výpočtech.
...
Kontrolou bylo zjištěno, že 287 nadbytečných záznamů představuje přesné duplicity celých řádků, a proto byly odstraněny.
...
- Shipping_Cost a Weight_Kg - raw data obsahovala rozdílné desetinné oddělovače, hodnoty byly standardizovány a následně převedeny na desetinné číslo.
- Returned_Flag - zdroj obsahoval více zápisů logických hodnot (TRUE/FALSE, Yes/No, 1/0), hodnoty byly sjednoceny a následně převedeny na datový typ pravda/nepravda.
...
- Oříznout;
- Vyčistit;
- sjednocení velikosti písmen tam, kde to bylo vhodné.

ID sloupce byly standardizovány na velká písmena.

Pro číselné atributy byla provedena základní business validace viz níže.

## Klíče

Byly prověřeny hlavní identifikátory:
- `Shipment_ID` - po odstranění přesných duplicit ověřen jako unikátní;
- `Carrier_ID`;
- `Warehouse_ID`;
- `Region_ID`;
- `Service_ID`.
...
Neplatné referenční klíče nebyly automaticky opravovány ani mazány. Byly označeny pomocí dalších validačních sloupců:
- `Carrier_Key_Valid`;
- `Warehouse_Key_Valid`;
- `Region_Key_Valid`;
- `Service_Key_Valid`.
...
Problematické záznamy nebyly automaticky přepisovány a byly označeny validačními flagy.
...
Nalezené nekonzistence jsou ponechány dohledatelné pomocí validačních flagů a nebyly automaticky opravovány bez znalosti správné zdrojové hodnoty.
...
Merge s referenčními tabulkami byl prováděn jako left outer, aby během validace nebyly z hlavní faktové tabulky automaticky odstraněny zásilky s neplatným referenčním klíčem.
...
Do `fact_shipments_clean` byly pomocí left outer sloučení připojeny referenční tabulky:
- `dim_carriers`;
- `dim_warehouses`;
- `dim_regions`;
- `dim_service_levels`.
...
Zdroj `complaints.csv` má granularitu `1 řádek = 1 stížnost`.

Protože jedna zásilka může mít více stížností, byl vytvořen samostatný dotaz `agg_complaints_by_shipment`.

Stížnosti byly seskupeny podle `Shipment_ID` a byla vytvořena metrika `Complaint_Count`.
...
Zásilky bez evidované stížnosti mají Complaint_Count = 0.
...
Hlavním analytickým výstupem přípravy dat je `fact_shipments_clean`.

Tento dotaz je načten jako připojení a současně přidán do **Excel data modelu**.

Ostatní technické, referenční a pomocné dotazy jsou ponechány jako **connection only**, aby se zbytečně nenačítaly do listů.
